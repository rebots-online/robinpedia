...
class ContentExtractor {
  ...
  /// Cluster manager for accessing compressed content
  final EnhancedClusterManager? _clusterManager;

  /// Create a content extractor
  ContentExtractor({required this.memoryManager, this._clusterManager}) {
  ...
  Future<Uint8List> extractBlob(DirectoryEntry entry) async {
    if (_clusterManager == null) {
      throw StateError('ClusterManager not initialized');
    }
    ...
    // Extract blob data
    return Uint8List.sublistView(cluster.data, blobOffset, nextBlobOffset);
  }

  /// Extract content for a directory entry
  Future<dynamic> extractContentFromEntry(DirectoryEntry entry) async {
    // Handle redirects
    if (entry.isRedirect) {
  ...
  Future<Article> extractArticle(DirectoryEntry entry) async {
    ...
    // Extract content
    final content = await extractContentFromEntry(entry) as String;
  ...
  Future<String> extractText(DirectoryEntry entry) async {
    final content = await extractContentFromEntry(entry);
